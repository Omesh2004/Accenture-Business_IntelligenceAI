...
export const dashboardAPI = {
  ...
  /* ═══════════════════════════════════════════════════════════════════════════
     Intelligence Layer — API Methods
     These endpoints will be served by the pipeline backend once built.
     Until then, each method returns a graceful empty-state default.
     IMPORTANT: These are NOT in the 15-second dashboard batch (useDashboard).
     They are fetched via the separate useIntelligenceData hook.
     ═══════════════════════════════════════════════════════════════════════════ */

  /* ─────────────── Trust Gate ─────────────── */

  /** Fetch trust gate verdicts for metrics within a time range */
  async getTrustVerdicts(tenants: string[], range: string): Promise<TrustVerdict[]> {
    try {
      const response = await apiClient.get<TrustVerdict[]>(
        `/trust/verdicts?tenants=${tenants.join(',')}&range=${range}`
      );
      if (response.data && !Array.isArray(response.data)) {
        return (response.data as any).data || (response.data as any).items || [];
      }
      return Array.isArray(response.data) ? response.data : [];
    } catch {
      console.debug('[Intelligence] Trust verdicts endpoint not available yet');
      return [];
    }
  },
  ...
  /** Fetch detected anomalies, optionally filtered by status */
  async getAnomalies(tenants: string[], range: string, status?: string): Promise<Anomaly[]> {
    try {
      let url = `/anomalies?tenants=${tenants.join(',')}&range=${range}`;
      if (status) url += `&status=${status}`;
      const response = await apiClient.get<Anomaly[]>(url);
      if (response.data && !Array.isArray(response.data)) {
        return (response.data as any).data || (response.data as any).anomalies || (response.data as any).items || [];
      }
      return Array.isArray(response.data) ? response.data : [];
    } catch {
      console.debug('[Intelligence] Anomalies endpoint not available yet');
      return [];
    }
  },

  /** Fetch root causes for a specific anomaly */
  async getAnomalyRootCauses(anomalyId: string): Promise<RootCause[]> {
    try {
      const response = await apiClient.get<RootCause[]>(
        `/anomalies/${encodeURIComponent(anomalyId)}/root-causes`
      );
      return response.data;
    } catch {
      console.debug(`[Intelligence] Root causes for ${anomalyId} not available yet`);
      return [];
    }
  },
  ...
  /** Fetch forecasts, optionally for a specific metric and horizon */
  async getForecasts(tenants: string[], metric?: string, horizon?: string): Promise<Forecast[]> {
    try {
      let url = `/forecasts?tenants=${tenants.join(',')}`;
      ...
      console.debug('[Intelligence] Forecasts endpoint not available yet');
      return [];
    }
  },
  ...
  /** Fetch causal impact result for an intervention */
  async getCausalImpact(tenants: string[], interventionId: string): Promise<CausalImpactResult | null> {
    try {
      const response = await apiClient.get<CausalImpactResult>(
        `/causal-impact?tenants=${tenants.join(',')}&intervention_id=${encodeURIComponent(interventionId)}`
      );
      return response.data;
    } catch {
      console.debug('[Intelligence] Causal impact endpoint not available yet');
      return null;
    }
  },

  /** Fetch list of available interventions for the causal picker */
  async getCausalInterventions(tenants: string[], range: string): Promise<Array<{ id: string; label: string; start: string; end: string; segment: string }>> {
    try {
      const response = await apiClient.get(
        `/causal-impact/interventions?tenants=${tenants.join(',')}&range=${range}`
      );
      return response.data?.interventions || [];
    } catch {
      console.debug('[Intelligence] Causal interventions endpoint not available yet');
      return [];
    }
  },
  ...
  /** Fetch recommendations, optionally filtered by status */
  async getRecommendations(tenants: string[], status?: string): Promise<Recommendation[]> {
    try {
      let url = `/recommendations?tenants=${tenants.join(',')}`;
      if (status) url += `&status=${encodeURIComponent(status)}`;
      const response = await apiClient.get<Recommendation[]>(url);
      if (response.data && !Array.isArray(response.data)) {
        return (response.data as any).data || (response.data as any).recommendations || (response.data as any).items || [];
      }
      return Array.isArray(response.data) ? response.data : [];
    } catch {
      console.debug('[Intelligence] Recommendations endpoint not available yet');
      return [];
    }
  },

  /** Approve a recommendation with a required comment */
  async approveRecommendation(id: string, comment: string): Promise<{ status: string }> {
    try {
      ...
      console.error(`[Intelligence] Failed to approve recommendation ${id}`);
      return { status: 'error' };
    }
  },

  /** Reject a recommendation with a required comment */
  async rejectRecommendation(id: string, comment: string): Promise<{ status: string }> {
    try {
      const response = await apiClient.post<{ status: string }>(
      ...
      console.error(`[Intelligence] Failed to reject recommendation ${id}`);
      return { status: 'error' };
    }
  },
  ...
  /** Fetch structured, verified narrative for a report */
  async getStructuredNarrative(reportId: string): Promise<StructuredNarrative | null> {
    try {
      const response = await apiClient.get<StructuredNarrative>(
        `/narratives/${encodeURIComponent(reportId)}`
      );
      return response.data;
    } catch {
      console.debug(`[Intelligence] Narrative ${reportId} not available yet`);
      return null;
    }
  },

  /* ─────────────── Observe / Model Observability ─────────────── */

  /** Fetch model run audit trail, optionally filtered by stage */
  async getModelRuns(stage?: string, range?: string): Promise<ModelRun[]> {
    try {
      const params = new URLSearchParams();
      if (stage) params.set('stage', stage);
      if (range) params.set('range', range);
      const qs = params.toString() ? `?${params.toString()}` : '';
      const response = await apiClient.get<ModelRun[]>(`/model-runs${qs}`);
      return response.data;
    } catch {
      console.debug('[Intelligence] Model runs endpoint not available yet');
      return [];
    }
  },

  /** Fetch golden set evaluation results */
  async getGoldenSetResults(): Promise<GoldenSetResult[]> {
    try {
      const response = await apiClient.get<GoldenSetResult[]>('/golden-set-results');
      return response.data;
    } catch {
      console.debug('[Intelligence] Golden set results not available yet');
      return [];
    }
  },

  /** Fetch per-stage health metrics for observability dashboard */
  async getStageHealth(): Promise<StageHealthMetric[]> {
    try {
      const response = await apiClient.get<StageHealthMetric[]>('/model-runs/health');
      return response.data;
    } catch {
      console.debug('[Intelligence] Stage health endpoint not available yet');
      return [];
    }
  },

  /** Fetch rollout ladder status per capability */
  async getRolloutLadder(): Promise<RolloutLadderStatus[]> {
    try {
      const response = await apiClient.get<RolloutLadderStatus[]>('/model-runs/rollout-ladder');
      return response.data;
    } catch {
      console.debug('[Intelligence] Rollout ladder endpoint not available yet');
      return [];
    }
  },
};
