...
export default function InsightRail({ answer }: { answer: AgentAnswer }) {
  ...
  return (
    <motion.aside
      ...
      className="w-[300px] shrink-0 space-y-3 overflow-y-auto border-l border-slate-100
                 bg-slate-50/40 p-4"
      aria-label="Insight summary"
    >
      <span className="block text-[10.5px] font-semibold uppercase tracking-[0.14em] text-slate-500">
        Insight summary
      </span>

      {answer.kpi_id && (
        <section className="surface p-4">
          <span className="block text-[12px] text-slate-500">{pretty(answer.kpi_id)}</span>
          <div className="mt-1.5 flex items-baseline gap-2.5">
            <span className="num text-slate-900"
                  style={{ fontSize: 'var(--step-3)', fontWeight: 600, letterSpacing: '-0.02em' }}>
          ...
          {path.length > 1 && (
            <div className="mt-3" style={{ height: 108 }}>
      ...
      {drivers && drivers.series.length > 0 && (
        <section className="surface p-4">
          <span className="mb-3 block text-[10.5px] font-semibold uppercase tracking-[0.14em]
                           text-slate-500">
            Key drivers
          </span>
