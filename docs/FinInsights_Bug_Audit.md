...
> **Superseded in part, 2026-08-28.** A follow-up audit
> (`docs/INTELLIGENCE_LAYER_PREREQUISITES.md`) re-verified this document's claims by running the
> code. Most held. Four did not, and are corrected inline below at **J1**, **E5**, **H1** and
> **A1**. The remediation plan built on both is `docs/PROPOSAL.md`; the scheduled work is
> `docs/TASK.md`.
>
> The follow-up also found four things this sweep did not cover, all of which post-date it or
> needed the taxonomy chain to be executed rather than read: the `ReplacingMergeTree` engine
> change defeating `dedup_integrity`; the absent `_simulated` marker; the missing ingest watermark
> on `investigations`; and the fact that two of the four tracker modules are never called.

> **Fixed since, verified against the working tree 2026-08-29.** Gates P0, P1 and most of P3
> closed these. Do not re-report them:
>
> **A3, A8** (real latency plumbed, `_simulated` read by the KPI card) · **B6** (canonical name at
> rest) · **C1** (session-grain funnels, stable anonymous id) · **D1, D2, D3, D4, D6** (one
> definition of a day, symmetric bounded windows, timestamp validator, clamped `parse_range`) ·
> **E1, E2, E3, E4** (worker pause/resume, dirty-flag commit, partition key, partitions) ·
> **E7** (`raw_rows` in the MV — decision D1) · **F2** (parameterised toggles SQL) · **F4**
> (forced cross-tenant union removed) · **F7** (CORS credentials) · **F8** (middleware order) ·
> **G1** (`storage/migrate.py`) · **G5** (`--reload` dropped) · **G6** (ClickHouse healthcheck) ·
> **G7** (`TZ=UTC`) · **G9** (root scratch files deleted) · **H3** (error rate) · **H4**
> (`pct_change` zero baseline) · **I1, I2, I3, I4** (client close, poller, connection-set snapshot,
> `finally: disconnect`) · **J2** (forwarder outcome counters + `/health/forwarder`) · **J3**
> (timestamp bounds).
>
> **Still open, and still the highest-value items:** **A1/A2** (mitigated by the `_simulated`
> marker, not removed — the values are still invented), **A4/A5** (a ratio KPI scored on its
> numerator), **A6/H5** (invented metrics published as measurements), **F1/F3/F5/F6** (identity
> and transport auth), **H1** (duplicate `/insights`), **H9** (entitlement source), **E9**
> (`seed_safexbank.py`), **G8** (no CI), and all of Section K.
>
> New findings from the 2026-08-29 doc/code re-audit are **Section L**.
...
> **Status 2026-08-28: this fix has not been made, despite being recorded as done elsewhere.**
> `docs/INTELLIGENCE_LAYER_PROPOSAL.md` §5 stated the `_simulated` marker had landed; a repo-wide
> search returns one hit, `contracts/pro_revenue.yaml`'s unrelated `is_simulated`. Two additions
> to the analysis above:
>
> - **The "real geo" fallback is not available.** `NexaBank/frontend/lib/tracker.ts` — the path
>   that would supply browser-derived location — has no call site, so there is no real signal to
>   derive from today. `POST /events/location` is the only one, and it fires once per session at
>   most.
> - **The session-invariance test cannot substitute for the marker.** `getSessionProfile` caches
>   the dice roll per session, so a fabricated dimension is perfectly session-invariant and passes
>   that test cleanly. `docs/INTELLIGENCE_LAYER_PROPOSAL.md` §6 describes the two gates as
>   redundant; they are not. The marker is load-bearing.
>
> Scheduled as `docs/TASK.md` P0-8.
...
### A4. The "simulated" honesty flag on the Avg Response Time KPI can never fire. P0 — **RESOLVED (P0-9)**
`api/main.py` `/metrics/kpi` counted a value as synthesised when `JSONHas(metadata,'response_time_ms')` was false. But the producer always set that key (to the random value). So `synthesised = 0` and the card rendered `simulated: false` on a value that is 100% synthetic. The safeguard was defeated by the producer that created the problem.

Fixed as A3+A4 together: the frontend field is `response_time_ms` (both spellings accepted), the producer lists `response_time_ms` in `metadata._simulated` when it had nothing real to write, and `/metrics/kpi` reads that marker instead of inferring from key absence.

### A5. `/metrics/kpi` also synthesises latency a second time, in SQL. P1 — **partly mitigated**
`15 + (cityHash64(event_name, toString(timestamp)) % 285)` is still in the query as the fallback when the key is absent, so two independent fabrication layers remain stacked on the same metric. What changed: the `has_measured` term now reads `metadata._simulated`, so the `simulated` count is real. The previous-period query still reports no `simulated` count, so the percentage-change arrow can still compare fabricated against fabricated.
...
### A8. A ratio KPI is scored on its numerator and narrated under the ratio's name. P0

*(Renumbered from A4 on 2026-08-29 — Section A already had an A4. `CLAUDE.md` and
`docs/HANDOFF.md` cite this finding as A8.)*

**[verified by running code and querying live data, 2026-08-29]**

`orchestrator.py` hands Detect `contract.scored_fundamental` — the numerator, because a rate is
non-additive and cannot be scored on itself. Nothing downstream records that substitution, so
Narrate labels the result with the contract's `name` and the agent published:

> Digital Adoption Rate rose 79.6% (urgent). It moved to 97.00 against an expected 54.00.

Every figure there traces to a stored row and the numeric verifier passed it. It is still wrong.
97 and 54 are **daily counts of digital transactions**, not an adoption rate. The real rate over
that window:

```sql
SELECT toDate(occurred_at) d, countIf(channel IN ('WEB','MOBILE'))/count() rate
FROM feature_intelligence.fact_transactions FINAL
WHERE tenant_id='nexabank' AND occurred_at >= '2026-08-15' AND occurred_at < '2026-08-29'
GROUP BY d ORDER BY d      -- 1.000 on all 14 days
```

The adoption rate **did not move at all**. Every transaction in `fact_transactions` is already
WEB or MOBILE. What moved was transaction volume, and volume is the numerator of a ratio whose
denominator moved with it. A reader acting on "digital adoption is up 80%" would be acting on a
metric that has been pinned at its ceiling for the whole window.

This is the failure mode CLAUDE.md rule 13 names: ranked, confident and meaningless, because it
looks correct. The verifier cannot catch it — the verifier checks that a number traces to a row,
not that the row is the quantity the sentence names.

**Partially mitigated, 2026-08-29.** `api/intelligence/phrasing.py` now states the substitution in
every answer ("The figure scored is digital transactions, the additive count Digital Adoption Rate
is built from, not the rate itself"), and `Contract.scored_fundamental` makes the orchestrator and
the phrasing layer read one field so they cannot drift apart again. That makes the answer honest.
It does not make the KPI measured.

**Still open.** Two options, and they are not equivalent:
1. Compute the ratio as a derived series (numerator/denominator per day) and score *that*, keeping
   Localize on the fundamentals where additivity actually holds. This is what the contract's
   `formula` already declares and what a reader assumes is happening.
2. Rename the contract to what is being scored. Cheaper, and defensible for a volume KPI, but it
   abandons every ratio contract on the platform — `kyc_completion_rate` and `loan_approval_rate`
   are scored the same way.

Option 1 is the right one; it is a Detect/orchestrator change, not a rendering one.

### A9. `digital_adoption_rate` is not recognised as a ratio by its own contract. P1

*(Renumbered from A5 on 2026-08-29.)*

`Contract.is_ratio` requires two fundamentals carrying `event`/`events` keys. This contract's
fundamentals are declared against `fact_transactions` with `table`/`measure`/`filter` and no
`event`, so `is_ratio` is `False` and `numerator()` returns `None` for a KPI whose `unit` is
literally `ratio`. Nothing depends on it today only because `scored_fundamental` falls through to
`fundamentals[0]`, which happens to be the numerator. A contract that listed its denominator first
would be scored on the denominator with no error anywhere.
...
> **Correction to E5, 2026-08-28.** The reasoning above is right, and it has a consequence this
> entry treats as purely benign. `ReplacingMergeTree` collapsing a replayed row is exactly what
> makes `count() == uniqExact(event_id)` — `contracts/kyc_completion_rate.yaml`'s `dedup_integrity`
> hard invariant, and the whole of demo scenario 1 — **stop being a stable test**. Reads stay
> correct; the *detector* is what breaks. See `docs/PROPOSAL.md` §2 Block D-ii, decision **D1**.
...
> **Corrected 2026-08-28: the second implementation is not unreachable.** It is shadowed as an
> HTTP route but live as a Python function, and `api/main.py:2082` calls it by name —
> `/admin/app/{id}/summary` does `get_insights(tenants=tenant_id, range=range)["insights"]` and
> gets the rule-based variant. So HTTP callers get the cached LLM one and the admin summary gets
> the other, which is the arrangement `CLAUDE.md` warns against deleting blindly. Both are used.
> Resolve it while writing the new `/ai_report` reader, not before.
...
### J1. The dashboard's own tracker sends no `event_id`. ~~P1~~ → **P3 (corrected)**

> **Corrected 2026-08-28.** The defect is real but the impact is not. `useAutoTrack` is imported
> by nothing and `tracker` is imported by nothing, so **this tracker never sends a single event**.
> The predicted "large pile" in `events_dead_letter WHERE stage='ingest_validation'` does not
> exist. It is a loaded gun, not a live wound — the 422 would begin the moment anyone wires it.
> Either wire it correctly or delete both files (`docs/TASK.md` P3-16).
...
4. ~~**A1/A2/A3/A4**~~ **done** — simulated fields are marked in the event envelope and the contract loader refuses them in `dimensions.allowed`. "We detect our own simulated data and refuse to localize on it" is a stronger demo than a plausible-looking false root cause.
...
17. Everything else, plus CI that runs the 28 test files already written. The harness exists
    (`docker compose --profile test run --rm tests`, and the `e2e` service); nothing runs it on a push.

---

## Section L — Intelligence layer (2026-08-29 re-audit)

Found by re-reading `docs/` against `api/intelligence/`. These are defects in the layer itself, not
in the substrate it reads.

### L1. An unentitled persona gets no insight row at all, not a filtered one. P1
`Orchestrator._narrate` does `claims, restricted = narrate.apply_entitlement(...)` then
`if restricted: continue`. So when a contract's `visible_to` excludes a persona, that persona's
`insights` row is never written. The design (`docs/PIPELINE_CONTRACT.md` §7) is that entitlement
filters the *claim set* and the persona still gets a narrative over what remains. The Gate D
assertion "restricted claims are **absent** from the ops-manager claim set" passes trivially when
the whole claim set is absent, so the leak test is weaker than it reads.

### L2. Propagation is half-built and the missing half is the one that suppresses alerts. P1
`Orchestrator.sweep` orders upstream KPIs first and threads `upstream_anomaly` into Causal, which
is what lets a declared edge reach `corroborated_cause`. Nothing sets `anomalies.status =
'explained'`, `anomalies` has no `explained_by` column, and contract cycles are not detected at load
time. So N alerts from one upstream cause are still N alerts. `docs/PIPELINE_CONTRACT.md` §3b
describes the whole pass; only steps 1 and 3 exist.

### L3. A truncated Localize search is invisible at rest. P2
`LocalizeResult.search_truncated` is set when a `cell_deltas` query raises, and `note` carries
"movement not explained by the available dimensions". Neither is written: `root_causes` has no
column for either, and no caller reads them. `docs/PIPELINE_CONTRACT.md` §3c's "never silently
truncate" is therefore honoured in memory and lost on write.

### L4. Coextensive-cell dedup keys on a rounded delta and can drop a distinct cell. P2
`stages/localize.py` treats two cells with the same `round(delta, 6)` as the same rows under
different names and keeps only the first. Two genuinely distinct cells whose movements happen to be
equal collapse, and the second never appears in the ranking. The intent (drop
`{txn_type=PAYMENT}` against `{region=NE, txn_type=PAYMENT}` when they cover the same rows) is
right; the test for it is a coincidence.

### L5. Tier 0 discovery failure is indistinguishable from "nothing discovered". P2
`contracts.load_all` wraps `discover_tier0` in a bare `except Exception: pass`. A ClickHouse outage,
a renamed column or a bad window all produce the same observable as a genuinely empty result: the
sweep runs over declared contracts only, with no log line and no `trust_findings` row. Coverage
silently drops to ten KPIs and nothing says so.

### L6. Detect does not suppress on a provisional window. P2
Contracts declare `provisional_window_minutes` and Trust Gate folds it into the grain-scaled
freshness floor, but `stages/detect.py` has no notion of a provisional window. The simulate console
backdates events, so a window can gain rows after it was scored, and a half-arrived window reads as
a phantom drop. `docs/EDGE_CASES.md` 00/02 require the suppression.

### L7. The determinism gate covers seven of the ten Signal Store tables. P2
`scripts/run_intelligence_gates.py`'s `SIGNAL_TABLES` diffs `investigations`, `trust_findings`,
`anomalies`, `root_causes`, `causal_effects`, `recommendations` and `insights`. `forecasts`,
`model_runs` and `outcomes` are not diffed. `model_runs` carries `latency_ms`, which cannot be
byte-identical — but that is a reason to diff it with that column excluded, not to skip it, since
`engine_type`, `inputs_hash` and `verifier_pass` are exactly the determinism evidence.

### L8. `mark_source_failed` is called with an exception where it declares a string. P3
`api/intelligence/service.py` passes `exc` to `loaders.mark_source_failed(source_id, tenants, error:
str)`. It stringifies by accident downstream; the annotation and the call disagree.

### L9. `docs/audits/` is cited 50 times in source and does not exist. P3
Fifty comments across `api/`, `core/`, `ingestion/`, `processing/` and `storage/` cite
`docs/audits/clickhouse_pipeline_audit_phase1_findings.md` and
`..._phase3_proposals.md` as the rationale of record for a specific engine choice, column default or
