...
Nothing in the Deferred column is a Phase 1 dependency. See CLAUDE.md rule 2.
...
| Li et al., **PSqueeze** | Multiple simultaneous causes; extends localization to **derived/ratio** measures via their fundamental components; robust as forecast-residual error grows. | Deferred — the fundamentals-not-ratios rule is implemented; the algorithm is not |
...
| Source | Justifies | Status |
|---|---|---|
| Amazon Science, **Chronos / Chronos-2** | Zero-shot probabilistic forecasting; ~120M params, 300+ forecasts/sec on one GPU. | Deferred |
