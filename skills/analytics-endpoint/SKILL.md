...
`api/main.py` is ~3,615 lines: all endpoints plus `RBACMiddleware`, SQL embedded in handlers, no
...
- **The duplicate `/insights` route.** `@app.get("/insights")` is declared twice: one handler
  (~:571) is cache/DB-backed LLM insights, the other (~:3430) is pure rule-based SQL. FastAPI
  serves the FIRST to HTTP callers, but the Python name `get_insights` resolves to the SECOND,
  which `/admin/app/{id}/summary` calls directly. Do not add a third, and do not delete either
  without tracing both call paths. For Phase 1, resolve this (one implementation) before extending.
